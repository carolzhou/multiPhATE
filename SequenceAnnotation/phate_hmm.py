...
class multiHMM(object):
    ...
    def setHmmProgram(self,hmmProgram): #*** Only jackhmmer is currently in service
        if(hmmProgram.lower() == 'hmmalign'):
            ...
            if PHATE_WARNINGS == 'True':
                print("phate_hmm says, WARNING:  Unrecognized HMM program:", hmmProgram) 
    ...
    def hmm1fasta(self,fasta,outfile,database,dbName): # fasta is a phate_fastaSequence.fasta object
        ...
        if self.outputFormat == XML or self.outputFormat == LIST:
            if PHATE_WARNINGS == 'True':
                print("phate_hmm says, ERROR:  only TBL format is currently being used for jackhmmer output parsing")
    ...
    def runHmm(self,fastaSet,dbType="protein"): # fastaSet is a phate_fastaSequence.multiFasta object
        ...
        if dbType.lower() == "protein" or dbType.lower == "aa" or dbType.lower == "prot" or dbType.lower == "peptide":
            ...
            if PHATE_WARNINGS == 'True':
                print("phate_hmm says, WARNING: unrecognized database type in runBlast:", dbType)
            return
        ...
        if PROTEIN:
            ...
            if self.UNIPARC_HMM: 
                ...
                count = 0
                if PHATE_PROGRESS == 'True':
                    print("phate_hmm says, Running UNIPARC hmm search:", database, dbName)
                for fasta in fastaSet.fastaList:
                    count += 1
            ...
            if self.PVOGS_HMM:  
                ...
                if DEBUG:
                    print("phate_hmm says, DEBUG: There are", len(pVOGlines), "pVOG database lines to search")
                count = 0; countA = 0
                for fasta in fastaSet.fastaList:
                    ...
                    for annot in fasta.annotationList:
                        ...
                        for pVOG in annot.pVOGlist:  # There may be multiple annotations to inspect
                            ...
                            if match_good:
                                ...
                                if DEBUG:
                                    print("phate_hmm says, DEBUG/WARNING: unexpected pVOG identifier:", pVOG)        
